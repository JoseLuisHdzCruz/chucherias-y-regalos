    ...
    const CartContextProvider = (props) => {
      ...
      const fetchCartFromAPI = async (customerId) => {
        try {
          const response = await axios.get(`https://backend-c-r.onrender.com/cart/${customerId}`);
      ...
      const addCartItem = async (newProduct) => {
        try {
          ...
          await axios.post("https://backend-c-r.onrender.com/cart/", newCartItem);
        } catch (error) {
          console.error("Error adding item to cart:", error);
        }
      };
      ...
      const updateCartItem = async (productId, customerId, quantity) => {
        try {
          await axios.put(`https://backend-c-r.onrender.com/cart/${customerId}/${productId}`, {
            cantidad: quantity,
          });
        } catch (error) {
      ...
      const deleteCartItem = async (productId) => {
        try {
          ...
          await axios.delete("https://backend-c-r.onrender.com/cart/", { data: cartDelete });
        } catch (error) {
          console.error("Error removing item from cart:", error);
        }
      };
      ...
      const clearCart = async () => {
        try {
          await axios.delete(`https://backend-c-r.onrender.com/cart/clear/${decodedToken.customerId}`);
          setCart([]);
        } catch (error) {
