...
const AdminKeyVerifly = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      const response = await axios.post(
        "https://backend-c-r-production.up.railway.app/admin/keyCompare",
        {
          correo: correo,
          clave: clave,
        }
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          navigate(`/admin/changePassword/${role}/${correo}`);
        }, 3000);
      } else {
  ...
  const reenviarClave = async () => {
    try {
      ...
      await axios.post(
        "https://backend-c-r-production.up.railway.app/admin/forgotPassword",
        {
          correo,
          role
        }
